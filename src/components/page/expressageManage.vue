  ...
  <div class="carManager">
    ...
    <div class="container">
      <div class="handle-box">
        <el-button type="primary" @click="addVisible = true">添加快递网点</el-button>
      </div>
      <el-table :data="tableData" border class="table" ref="multipleTable">
        <el-table-column prop="campusName" label="学校" width="500" class-name="table"></el-table-column>
        <el-table-column prop="pickupAddress" label="快递网点" width="500" class-name="table"></el-table-column>
    ...
    <el-dialog title="添加" :visible.sync="addVisible" width="30%">
      <el-form ref="addForm" :model="addForm" label-width="100px">
        <el-form-item label="学校名" prop="campusName">
            	<!-- 学校是下拉框 -->
		    <el-select v-model="addForm.campusId" :placeholder="addForm.campusName" class="handle-select">         
                <el-option v-for="item in schoolNames" :key="item.id" :label="item.campusName" :value="item.id"></el-option>
            </el-select>  
        </el-form-item>
        <el-form-item label="快递网点" prop="pickupAddress">
          <el-input v-model="addForm.pickupAddress"></el-input>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="编辑" :visible.sync="editVisible" width="30%">
      <el-form ref="editForm" :model="editForm" label-width="100px">
        <el-form-item label="学校名" prop="campusName">
		  	<!-- 学校是下拉框 -->
		    <el-select v-model="editForm.campusId" :placeholder="editForm.campusName" class="handle-select">         
          <el-option v-for="item in schoolNames" :key="item.id" :label="item.campusName" :value="item.id"></el-option>
        </el-select>                             
        </el-form-item>
        <el-form-item label="快递网点" prop="pickupAddress">
          <el-input v-model="editForm.pickupAddress" class="handle-select"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      this.$axios({
      	url: 'http://39.99.137.77:8081/projects/pickup/selectAll?pageNum='+res.cur_page+'&pageSize='+ res.pagesize,
        method: 'get',
      	data: { }
      })
      	.then(function(response) {
            ...
            for(var i=0;i<mm.length;i++) {
                  res.tableData.push({
                    campusName: mm[i].campusName,
                    campusId: mm[i].campusId,
                    pickupAddress: mm[i].pickupAddress,
                    id: mm[i].id
                  })
            }
    ...
    saveAdd() {
      ...
      this.$refs.addForm.validate(valid => {
        if (!valid) {
          ...
          this.$axios({
            url: "http://39.99.137.77:8081/projects/pickup",
            method: "post",
            data: setData
          })
            .then(function(response) {
    ...
    deleteRow() {
      ...
      //console.log(id)
      this.$axios({
        url: "http://39.99.137.77:8081/projects/pickup?id="+id,
        method: "delete",
        data: {  }
      })
        .then(function(response) {
