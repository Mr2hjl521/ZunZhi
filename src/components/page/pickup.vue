<template>
  <div class="carManager">
    ...
    <div class="container">
      ...
      >
        <el-table-column prop="outTradeNo" label="订单号" align="center" class-name="table"></el-table-column>
        <el-table-column prop="takeDeliveryOrderId" label="订单ID" align="center" class-name="table"></el-table-column>
        ...
        <el-table-column prop="tPrice" label="交易金额" align="center" class-name="table"></el-table-column>
         <el-table-column prop="pPrice" label="实际交易金额" align="center" class-name="table"></el-table-column>
          <el-table-column prop="accPrice" label="余额支付金额" align="center" class-name="table"></el-table-column>
  ...
  <!-- 查看弹出框 -->
    <el-dialog title="查看" :visible.sync="editVisible" width="30%">
      <el-form ref=""  label-width="100px">
       <el-form-item label="" v-for="(item,index) in list" :key="index">
           <span>{{item.pickupAddress}}</span>   <span style="margin-left:10px;">{{item.logisticsName}}</span>  <span style="margin-left:10px;">{{item.pickCode}}</span> <span style="margin-left:10px;">{{item.name}}</span><span style="margin-left:10px;">{{item.phone}}</span>
       </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
     handleEdit(index, row) {
      this.idx = index;
      this.id = row.takeDeliveryOrderId;
       var res = this;
      this.$axios({
        url: "http://39.99.137.77:8081/projects/takeServiceOrder/viewParcelDesc?takeDeliveryOrderId="+res.id,
        method: "get",
        data: {}
      })
