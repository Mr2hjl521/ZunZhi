  ...
  <div class="carManager">
    ...
    <div class="container">
      <div class="handle-box">
        服务人员姓名:<el-input v-model="selectName" placeholder="请输入姓名" class="handle-inputBig mr10"></el-input>
      ...
      <el-table :data="tableData" border class="table" ref="multipleTable">
        ...
        <el-table-column prop="tpoStatus" label="订单状态" width="110" class-name="table">
          <template slot-scope="status">
            <div v-if="status.row.tpoStatus==0">待服务</div>
            <div v-if="status.row.tpoStatus==1">待评价</div>
            <div v-if="status.row.tpoStatus==2">已结算</div>
            <div v-if="status.row.tpoStatus==3">已删除订单</div>
            <div v-if="status.row.tpoStatus==4">已完成</div>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="againServiceUserName"  label="服务人员2" width="120" class-name="table">
           <template slot-scope="msg">
            <div v-if="msg.row.againServiceUserName!=='null'"> 
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      res.tableData=[];
      this.$axios({
      	url: 'http://39.99.137.77:8081/projects/testPhoneOrder/pcFindError?pageNum='+res.cur_page+'&pageSize='+ res.pagesize,
      	method: 'get',
      	data: { }
      })
    ...
    search() {
      ...
      this.$axios({
        url: 'http://39.99.137.77:8081/projects/testPhoneOrder/findSearchByServiceUserName?pageNum='+res.cur_page+'&pageSize='+res.pagesize+'&serviceUserName='+res.selectName,  
        method: "get",
        data: {
        }
      })
    ...
    deleteRow() {
      ...
      // this.$axios({
      //   url: "http://39.99.137.77:8081/projects/waterAgent/deleteByPrimaryKey",
      //   method: "post",
      //   data: { id: id }
      // })
      //   .then(function(response) {
